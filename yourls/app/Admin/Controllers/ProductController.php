...
class ProductController extends AdminController
{
	...
	protected function grid()
	{
		...
		$grid->column('id', __('產品ID'))->sortable();
		$grid->column('name', __('產品名稱'))->sortable();
		...
		$grid->filter(function ($filter) {
			...
			$filter->equal('id', '產品ID');
			$filter->like('name', '產品名稱');
	...
	protected function detail($id)
	{
		...
		$show->field('id', __('產品ID'));
		$show->field('name', __('產品名稱'));
	...
	protected function form()
	{
		...
		$form->text('name', __('產品名稱'))
			->creationRules(['required', "unique:product"])
			->updateRules(['required', "unique:product,name,{{id}}"]);
